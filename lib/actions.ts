...
async function captureCompressedScreenshot(page: Page, fullPage: boolean) {
  ...
  for (const quality of qualities) {
    ...
    if (buffer.length <= MAX_INLINE_SCREENSHOT_BYTES) return { buffer, quality };
  }
  return null;
}
  ...
  try {
    ...
    switch (step.action) {
      ...
      case 'scroll':
        ...
        else await page.evaluate(({ x, y }: { x: number; y: number }) => window.scrollBy(x, y), { x: step.x ?? 0, y: step.y ?? 500 });
      ...
      case 'extract_text': {
        ...
        result = { text: (text || '').slice(0, step.max_chars ?? 2000) };
        break;
      }
      case 'extract_html': {
        const selector = step.selector || 'body';
        const html = await page.innerHTML(selector, { timeout });
        result = { html: html.slice(0, step.max_chars ?? 5000) };
        break;
      }
      case 'extract_links': {
        const maxItems = step.max_items ?? 50;
        const links = await page.$$eval('a[href]', (els, limit) =>
          els.slice(0, limit as number).map(el => ({ href: (el as HTMLAnchorElement).href, text: el.textContent?.trim().slice(0, 100) || '' })),
          maxItems,
        );
        result = { links };
        break;
      ...
      case 'screenshot': {
        ...
        if (!captured) throw new Error(`Screenshot exceeds ${Math.round(MAX_INLINE_SCREENSHOT_BYTES / 1024)}KB after compression`);
      ...
      case 'evaluate_safe':
        switch (step.operation) {
          case 'title': result = { title: await page.title() }; break;
          case 'bodyHeight': result = { bodyHeight: await page.evaluate(() => document.body.scrollHeight) }; break;
          case 'elementCount': result = { count: await page.evaluate((sel: string) => document.querySelectorAll(sel).length, step.selector || '*') }; break;
          case 'performance':
            result = await page.evaluate(() => {
              const nav = performance.getEntriesByType('navigation')[0] as PerformanceNavigationTiming;
              return { domContentLoaded: Math.round(nav?.domContentLoadedEventEnd || 0), loadComplete: Math.round(nav?.loadEventEnd || 0) };
            });
            break;
          case 'visibility': result = { visible: await page.isVisible(step.selector || 'body') }; break;
        }
        break;
      case 'get_title': result = { title: await page.title() }; break;
      case 'get_url': result = { url: page.url() }; break;
      case 'get_viewport': result = { viewport: page.viewportSize() }; break;
      ...
      case 'validate_status': result = { url: page.url(), ok: true }; break;
      case 'capture_accessibility_snapshot': {
        const roles = await page.evaluate(() => {
          const implicitRoles: Record<string, string> = { A:'link',BUTTON:'button',FORM:'form',H1:'heading',H2:'heading',H3:'heading',HEADER:'banner',FOOTER:'contentinfo',MAIN:'main',NAV:'navigation',INPUT:'textbox',TEXTAREA:'textbox',OL:'list',UL:'list',LI:'listitem',ARTICLE:'article' };
          const els = document.querySelectorAll('[role],a,button,form,h1,h2,h3,header,footer,main,nav,input,textarea,ol,ul,li,article');
          return Array.from(els).slice(0,100).map(el => ({ role:el.getAttribute('role')||implicitRoles[el.tagName]||'generic', label:el.getAttribute('aria-label')||el.getAttribute('name')||el.textContent?.trim().slice(0,80)||'' }));
        });
        result = { roles, count: roles.length };
        break;
      }
      case 'capture_console': result = { console_errors: captures.consoleErrors.length, entries: captures.consoleErrors.slice(0,20) }; break;
      case 'capture_network_errors': result = { network_errors: captures.networkErrors.length, entries: captures.networkErrors.slice(0,20) }; break;
      default: throw new Error('Unknown action');
    }
    return { status:'pass', duration_ms:Date.now()-start, result };
  } catch (err) {
    return { status:'fail', duration_ms:Date.now()-start, error:(err as Error).message?.slice(0,500) };
  }
}
